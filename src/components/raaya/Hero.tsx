...
const HERO_IMAGE_DESKTOP = 'https://static.wixstatic.com/media/cef78c_34612791ed8d44dd9ba8de744220cd82~mv2.jpg'
const HERO_IMAGE_MOBILE  = 'https://static.wixstatic.com/media/cef78c_78d84901397e40fa843c40272f504a72~mv2.jpg'
...
// ── Hero Form Field ──────────────────────────────────────────
function HeroFormField({ label, name, type = 'text', placeholder, value, onChange, required, error }: {
  ...
  return (
    <div>
      <label style={{
        ...
        color: focused ? '#C8A14A' : 'rgba(242,237,228,0.6)',
      ...
      <input
        ...
        style={{
          ...
          borderBottom: `1px solid ${error ? '#ff6b6b' : focused ? '#C8A14A' : 'rgba(242,237,228,0.3)'}`,
          padding: '0.55rem 0', fontFamily: GAM,
          fontSize: '0.85rem', fontWeight: 300, color: '#F2EDE4',
...
// ── Single global enquire tab — fixed to viewport ──
function EnquireTab() {
  ...
  return (
    <button
      ...
      style={{
        ...
        transform: 'translateY(-50%) rotate(90deg) translateY(calc(100% - 2px))',
        transformOrigin: 'bottom right', zIndex: 200,
        background: '#2B2725', color: '#F4EFE8',
        fontFamily: GT, fontSize: '0.5rem', fontWeight: 400,
        letterSpacing: '0.22em', textTransform: 'uppercase' as const,
        padding: '0.55rem 1.5rem', border: 'none',
        borderBottom: '2px solid #C8A14A', cursor: 'pointer',
        whiteSpace: 'nowrap' as const,
        transition: 'background 0.25s ease, color 0.25s ease',
        pointerEvents: 'auto', userSelect: 'none' as const,
      }}
      onMouseEnter={e => { e.currentTarget.style.background = '#C8A14A'; e.currentTarget.style.color = '#2B2725' }}
      onMouseLeave={e => { e.currentTarget.style.background = '#2B2725'; e.currentTarget.style.color = '#F4EFE8' }}
    >
      Enquire Now
    </button>
  )
  ...
  return (
    ...
    >
      ...
      <div style={{ background: '#2B2725', padding: '1.5rem 2rem 1.25rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <img src={LOGO_SVG} alt="Runwal Raaya" style={{ height: '52px', width: 'auto', filter: 'brightness(0) invert(1)', opacity: 0.9, display: 'block', margin: '0 auto 1rem' }} />
      ...
      <div style={{ padding: '1.5rem 2rem 1.75rem' }}>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.1rem' }} className="hero-form-grid-2col">
              <HeroFormField label="Full Name" name="name" placeholder="Your name" value={form.name} onChange={handleChange} required error={errors.name} />
              <HeroFormField label="Email" name="email" type="email" placeholder="your@email.com" value={form.email} onChange={handleChange} required error={errors.email} />
            </div>
            <HeroFormField label="Phone" name="phone" type="tel" placeholder="+91 00000 00000" value={form.phone} onChange={handleChange} required error={errors.phone} />
            <button type="submit" onMouseEnter={() => setBtnHovered(true)} onMouseLeave={() => setBtnHovered(false)} disabled={isSubmitting}
              style={{
                width: '100%', padding: '0.9rem',
                background: btnHovered ? '#3a3330' : '#2B2725', color: '#F2EDE4',
                fontFamily: GT, fontSize: '0.58rem', fontWeight: 300,
                letterSpacing: '0.22em', textTransform: 'uppercase' as const,
                border: `1px solid ${btnHovered ? '#C8A14A' : 'rgba(200,161,74,0.18)'}`,
                cursor: isSubmitting ? 'not-allowed' : 'pointer', transition: 'all 0.3s ease',
                boxShadow: btnHovered ? '0 4px 20px rgba(200,161,74,0.1)' : 'none',
                marginTop: '0.1rem', opacity: isSubmitting ? 0.6 : 1,
              }}>
...
export default function Hero() {
  return (
    <>
      {/* Single enquire tab — page-level, outside hero DOM */}
      <EnquireTab />

      {/* Hero section */}
      <section style={{ width: '100%', position: 'relative', background: '#2B2725', overflow: 'hidden' }} className="hero-fullscreen">
        <motion.div
        ...
        >
          <picture style={{ display: 'block', width: '100%', lineHeight: 0 }}>
            <source srcSet={HERO_IMAGE_MOBILE} media="(max-width: 640px)" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={HERO_IMAGE_DESKTOP}
              alt="Runwal Raaya — Worli, Mumbai"
              className="hero-main-image"
              style={{ display: 'block', width: '100%', height: 'auto' }}
            />
          </picture>
          {/* Bottom vignette */}
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(to top, rgba(20,14,10,0.35) 0%, transparent 40%)' }} />
        </motion.div>
      </section>
    </>
  )
