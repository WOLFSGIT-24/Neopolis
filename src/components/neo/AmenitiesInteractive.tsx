...
import { Activity, Trophy, Heart, Coffee, Users, Sparkles, Check } from 'lucide-react'
...
export default function AmenitiesInteractive() {
  ...
  return (
    <section id="amenities" className="relative w-full py-24 sm:py-32 bg-[#121620] text-neo-ivory overflow-hidden border-t border-white/5">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-16 space-y-2.5 sm:space-y-3">
          <span className="text-[11px] sm:text-xs uppercase tracking-[0.3em] text-[#C98D78] font-medium block">
            03 • Curated Infrastructure
          </span>
          <h2 className="text-3xl sm:text-5xl md:text-6xl font-serif font-light text-white leading-tight">
            Designed around <br />
            <span className="italic text-neo-peach font-normal">the way you live.</span>
          </h2>
          <p className="text-xs sm:text-sm text-neo-muted max-w-xl mx-auto">
          ...
          {categories.map((cat, idx) => {
            ...
            return (
              <button
                ...
                className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs uppercase tracking-[0.15em] sm:tracking-[0.18em] font-medium transition-all duration-300 flex items-center gap-1.5 shrink-0 ${
                  isActive
                    ? 'bg-[#C98D78] text-[#10131A] shadow-lg shadow-[#C98D78]/20 font-semibold'
                    : 'bg-[#18243A]/60 text-neo-muted border border-white/10 hover:border-[#C98D78]/40 hover:text-white'
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-center rounded-2xl bg-[#10131A] border border-[#C98D78]/20 p-4 sm:p-8 lg:p-10 shadow-2xl">
          {/* Left Dynamic Image */}
          <div className="lg:col-span-6 relative aspect-[16/11] rounded-xl overflow-hidden border border-white/10 group">
            <Image
              src={current.image}
              alt={current.name}
              fill
            ...
            <div className="absolute bottom-5 left-5 right-5 p-3 rounded-lg bg-black/60 backdrop-blur-md border border-white/10">
              <span className="text-[10px] uppercase tracking-widest text-[#C98D78] block font-mono">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="space-y-4 pt-2">
              {current.items.map((item, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-xl bg-[#18243A]/40 border border-white/5 hover:border-[#C98D78]/30 transition-all flex items-start gap-3.5"
                >
                  <div className="w-6 h-6 rounded-full bg-[#C98D78]/20 flex items-center justify-center text-[#C98D78] shrink-0 mt-0.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neo-muted mt-0.5 leading-relaxed">
                      {item.detail}
                    </p>
                  </div>
                </div>
